  ...
  el.innerHTML = `
    <div style="padding:16px 20px;border-bottom:1px solid var(--gray-100);background:var(--gray-50);display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="display:flex;gap:6px">
        ${c.status==='open'?'<button class="btn btn-primary" style="font-size:12px" onclick="alert(\'처리 완료로 변경\')">✓ 처리 완료</button>':'<span class="badge badge-gray">완료</span>'}
    ...
    <div style="flex:1;padding:20px;overflow-y:auto">
      ...
      <textarea style="width:100%;padding:10px 12px;border:1px solid var(--gray-200);border-radius:var(--r);font-size:13px;font-family:inherit;resize:vertical;line-height:1.6;outline:none" rows="4" placeholder="답변 내용을 입력하세요..."></textarea>
      <div style="display:flex;justify-content:flex-end;gap:6px;margin-top:8px">
        <button class="btn" onclick="alert('임시 저장')">임시 저장</button>
        <button class="btn btn-primary" onclick="alert('발송 완료')">📤 발송</button>
      </div>
    </div>`;
}
